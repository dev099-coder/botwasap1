const  XBOT = 'LIGHTBOT'; // Nama Bot Whatsapp
...
conn.sendMessage(id, 'Sucesso Alterar Grupo de Nome' ,MessageType.text, { quoted: m } );
...
axios.get(`https://arugaz.herokuapp.com/api/howgay`).then((res) => {
	conn.sendMessage(id, '[WAIT] Processo...🔍', MessageType.text)
    let hasil = ` ${res.data.desc} \n\n *Percent Gay Lo !!!* _${res.data.persen}_`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
conn.sendMessage(id, 'enviar .foto de menina / menino \ n \ nExemplo: foto de menina' ,MessageType.text);
}
else if (text == '#menu'){
conn.sendMessage(id, ' 🔍Obrigado, usou o BOT * LIGHTBOT *, siga o Instagram Mimin Yah🔍� : https://instagram.com/@devjavastudios' ,MessageType.text);
}
else if (text == '#info'){
    ...
    axios.get(`http://inyourdream.herokuapp.com/glitch?kata1=${teks1}&kata2=${teks2}`).then((res) => {
      imageToBase64(res.data.status)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[ WAIT ] Sendo processado🔍 Por favor espere um momento', MessageType.text, { quoted: m })
  ...
  var teks = text.replace(/#igstalk /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.Profile_pic)
        .then(
          (ress) => {
           let hasil = `User Ditemukan!!\n\n*➸ Nome :* ${res.data.Name}\n*➸ Nome do usuário :* ${res.data.Username}\n*➸ Seguidores :* ${res.data.Jumlah_Followers}\n*➸ Segue :* ${res.data.Jumlah_Following}\n*➸ Número de Postagem :* ${res.data.Jumlah_Post}\n*➸ Bio :* ${res.data.Biodata}`;
            var buf = Buffer.from(ress, 'base64')
